...
class MessageSender:
	def __init__(self):
		...
		self.register("qmsg_key", self.qmsg)
	...
	def qmsg(self, qmsg_key, title, content):
	        assert type(qmsg_key) == str, "Wrong type for Qmsg token."
	        ...
	        try:
		        ...
		        print(resp_json)
		        if resp_json["code"] == 200:
			        print(f"【Qmsg】Send message to Qmsg successfully.")
		        if resp_json["code"] != 200:
			        print(f"【Qmsg】【Send Message Response】{response.text}")
			        return -1
